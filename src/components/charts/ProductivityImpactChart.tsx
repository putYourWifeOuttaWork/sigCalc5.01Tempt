...
interface ProductivityImpactChartProps {
  results: ErrorCalculationResult[];
}

export function ProductivityImpactChart({ results }: ProductivityImpactChartProps) {
  ...
  const chartData = React.useMemo(() => {
    ...
    results.forEach(result => {
      ...
      if (!dataMap.has(date)) {
        dataMap.set(date, {
          date,
          annualErrors: result.annualErrors,
          values: {}
        });
      }
      const cohortKey = `${result.errorType}-${result.errorArea}`;
      const value = showHoursMode 
        ? result.errorImpactHours * result.workingDays
  ...
  return (
    <div className="w-full h-[600px] bg-white p-6 rounded-xl shadow-sm">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {showHoursMode 
              ? 'Annual hours impact per user by error type and area'
      ...
      <ResponsiveContainer width="100%" height="87%">
        ...
        >
          <defs>
            <linearGradient id="errorBarGradient" x1="0" y1="1" x2="0" y2="0">
          ...
          <YAxis
            yAxisId="left"
            label={{ 
              value: showHoursMode ? 'Hours Impact/Year' : 'Financial Impact ($)', 
              angle: -90, 
              position: 'insideLeft', 
              offset: 10 
            }}
