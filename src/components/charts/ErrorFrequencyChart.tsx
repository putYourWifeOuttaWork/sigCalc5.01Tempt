...
export function ErrorFrequencyChart({ results }: ErrorFrequencyChartProps) {
  ...
  const chartData = React.useMemo(() => {
    ...
    results.forEach(result => {
      ...
      if (!dataMap.has(date)) {
        dataMap.set(date, {
          date,
          timePerError: result.distractionTimePerError,
          values: {}
        });
      }
      const cohortKey = `${result.errorType}-${result.errorArea}`;
      const value = showHoursMode 
        ? result.dailyDistractionHours * result.workingDays
  ...
  return (
    <div className="w-full h-[600px] bg-white p-6 rounded-xl shadow-sm">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h3 className="text-xl font-semibold text-gray-900">Error Impact Analysis</h3>
          <p className="text-sm text-gray-500">
            {showHoursMode 
              ? 'Annual hours lost per user by error type and area'
      ...
      <ResponsiveContainer width="100%" height="87%">
        ...
        >
          <defs>
            <linearGradient id="timePerErrorGradient" x1="0" y1="1" x2="0" y2="0">
          ...
          <YAxis
            yAxisId="left"
            label={{ 
              value: showHoursMode ? 'Hours Lost/Year' : 'Financial Impact ($)', 
              angle: -90, 
              position: 'insideLeft', 
              offset: 10 
            }}
          ...
          <Bar
            yAxisId="right"
            dataKey="timePerError"
            name="Time per Error"
            fill="url(#timePerErrorGradient)"
            opacity={0.6}
            barSize={20}
          />
        </ComposedChart>
